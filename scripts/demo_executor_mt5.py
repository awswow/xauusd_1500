...
class ExecCfg:
    ...
    # Heartbeat frequency (seconds). Keep at 60 for watchdog.
...
def main():
    ...
    logger.log(
        ...
        symbol,
        timeframe,
        {"config": {"strategy": sp, "execution": exec_cfg.__dict__, "risk": risk_cfg}},
    )
    ...
    while True:
        try:
            # Heartbeat (prevents watchdog false alerts)
            if (utc_now() - last_heartbeat).total_seconds() >= int(exec_cfg.heartbeat_sec):
            ...
            if realized <= -abs(exec_cfg.daily_loss_limit_usd):
                logger.log(
                    ...
                    symbol,
                    timeframe,
                    {"realized_usd": realized, "limit": exec_cfg.daily_loss_limit_usd},
                )
            ...
            if trades_today >= int(exec_cfg.max_trades_per_day):
                logger.log(
                    ...
                    symbol,
                    timeframe,
                    {"trades_today": trades_today, "cap": exec_cfg.max_trades_per_day},
                )
                time.sleep(args.poll_sec)
                continue

            # If we already have an open position, do nothing (1-position model)
            ...
            # Use the last CLOSED bar for signaling
            ...
            if cooldown_bars and last_entry_bar_time is not None:
                ...
                if last_closed_bar.time_utc - last_entry_bar_time < min_gap:
                    logger.log(
                        ...
                        symbol,
                        timeframe,
                        {"bar_time": iso(last_closed_bar.time_utc), "last_entry_bar_time": iso(last_entry_bar_time)},
                    )
                    time.sleep(args.poll_sec)
                    continue
            ...
            # Never die on transient MT5 / network / data issues
            logger.log("executor_exception", symbol, timeframe, {"error": repr(e)})
